...
def post_process_prediction(hypo_tokens, src_str, alignment, align_dict, tgt_dict, remove_bpe):
    ...
    if align_dict is not None or remove_bpe is not None:
        ...
        hypo_tokens = tokenizer.Tokenizer.quetion_tokenize(hypo_str, tgt_dict, add_if_not_exist=True)
...
def make_positions(tensor, padding_idx, left_pad):
    ...
    positions = torch.arange(padding_idx + 1, max_pos).type_as(tensor)
    positions = positions.expand_as(tensor)
    if left_pad:
